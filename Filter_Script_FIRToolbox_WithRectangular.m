...
Hd = FIR_Bandstop_Toolbox_RectangularWin
channel = step(Hd,raw_data);

t=0:1/256:(length(raw_data)-1)/256;
figure(2);
subplot(2,1,1);
plot(t,channel); title('Bandstop Filter Applied; Rectangular Window'); xlabel('Time'); ylabel('Microvolts');

%% Filter Data: Bandpass

Hd = FIR_Bandpass_Toolbox_RectangularWin
new = step(Hd,channel);
...
figure(2);
subplot(2,1,2);
plot(t,new1); title('Bandstop and Bandpass Filter Applied; Rectangular Window'); xlabel('Time'); ylabel('Microvolts');
