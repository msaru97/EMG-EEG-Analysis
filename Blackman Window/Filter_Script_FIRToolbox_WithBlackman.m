...
y = fft(raw_data);
figure(1);
subplot(2,1,2); suptitle('Index Finger; Channel 3')
dw_5(15346)=[];
plot(dw_5, y); title('FFT'); xlabel('Frequency(Hz)'); ylabel('Amplitude');

%% Filter Data: Bandstop

Hd = FIR_Bandstop_Toolbox_BlackmanWin
channel = step(Hd,raw_data);

t=0:1/256:(length(raw_data)-1)/256;
figure(2);
subplot(2,1,1);
plot(t,channel); title('Bandstop Filter Applied; Blackman Window'); xlabel('Time'); ylabel('Microvolts');

%% Filter Data: Bandpass

Hd = FIR_Bandpass_Toolbox_BlackmanWin
new = step(Hd,channel);
...
figure(2);
subplot(2,1,2);
plot(t,new1); title('Bandstop and Bandpass Filter Applied; Blackman Window'); xlabel('Time'); ylabel('Microvolts');
suptitle('Index Finger Data; Channel 3')
