function [data_processed] = FIR_Bandpass(wc1, wc2, ~, delta_t, M, time, ~, zero_pad, data_processed)
...
for n = 1:time
    for k = -M:M
     h_k = ((((delta_t*wc2)/pi)* sinc(wc2*k*delta_t)) - (((delta_t*wc1)/pi)* sinc(wc1*k*delta_t))); %filter coefficient for low-pass filter
